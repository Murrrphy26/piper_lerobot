...
- `PYTHONPATH=src python -m piper_train.record_episode --dataset-format lerobot --task pick_cube --action-source follower --duration 30` 可成功生成 dataset
...
- `python -m piper_train.record_episode --camera-indices 2,4,0 --camera-names cam_top,cam_left,cam_right` 能同时保存 3 路视频
...
1. 用 `python -m piper_train.record_episode` 录 5 到 10 条抓方块短 episode。
